...
public class ConfigMisc {
    ...
    public void setConfigFromSysex(byte[] sysex) {
        ...
        if (sysex.length >= Constants.MD_SYSEX_MISC_SIZE) {
            ...
            short flags = Utils.sysex2short(sysex_short);
            sysex_short[0] = sysex[i++];
            sysex_short[1] = sysex[i++];
            sysex_short[2] = 0;
            sysex_short[3] = 0;
    ...
    public byte[] getSysexFromConfig() {
        ...
        int i = 0;
        short flags = (short) (((all_gains_low) ? 1 : 0) |
        ...
        byte[] sysex_byte = Utils.byte2sysex((byte) getNoteOff());
        sysex[i++] = sysex_byte[0];
        sysex[i++] = sysex_byte[1];
        sysex_byte = Utils.byte2sysex((byte) latency);
        sysex[i++] = sysex_byte[0];
        sysex[i++] = sysex_byte[1];
        byte[] sysex_short = Utils.short2sysex(flags);
